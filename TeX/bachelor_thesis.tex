...
\begin{equation}
	\phi(u) = \operatorname{max}\{u_{i, \mu, \nu} | (j-1) \cdot k_1 < \mu \leq j \cdot k_1, (k-1) \cdot k_2 < \mu \leq k \cdot k_2 \}.
\end{equation}
...
\begin{equation} \label{softmax}
	\forall i \in \{1, 2, ..., p_1\} \quad \phi (u)_i = \frac{e^{a_i}}{\sum_{j=1}^{p_1} e^{a_j}}.
\end{equation}
...
Algoritmus \emph{AdaGrad} dle \cite{AdaGrad} přizpůsobuje řád učení každému parametru

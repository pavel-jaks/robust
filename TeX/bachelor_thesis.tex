...
\end{equation}
kde $\kappa$ je malé číslo.
Takovým vzorkům $\tilde{x} = x + \Delta x$ se říká \emph{adversariální vzorky}.

Obecně tedy množinu adversariálních vzorků lze definofat jako
\begin{equation}
	\widetilde{\mathbb{X}} = \{\tilde{x} \in \mathbb{R}^n
	| (\exists x, y \in \mathbb{R}^n \times \mathbb{R}^m)
	(F_\theta(x) = y \land F_\theta(\tilde{x}) \neq y \land \|\tilde{x} - x\| < \kappa)\}.
\end{equation}
...
Ta využívá mnoho-dimenzionální lineární vztahy neuronové sítě \cite{CSI_algo}
a má předpis:
...
Třetí metoda nahlíží na generování adversariálních vzorků jako na optimalizační úlohu
\cite{szegedy2014intriguing}, \cite{L_BFGS}:
\begin{equation} \label{optim_adv}
	\tilde{x} = \arg \min_{\hat{x} \in D_{F_\theta}} \lambda \cdot \|\hat{x} - x\| + L(F_{\theta}(\hat{x}), y^{adv}),
\end{equation}
kde $\lambda > 0$, $D_{F_\theta}$ je definičním oborem $F_\theta$, $y^{adv}$ značí záměrně nesprávnou značku.
...
Další metoda nese název \emph{PGD} (zkratka angl. \emph{projected gradient descent}).
...
% \subsection{C\&W}
Následující metoda má opět optimalizační charakter. Jmenuje se \emph{CW} (\emph{Carlini-Wagner})
a má předpis \cite{L_BFGS}, \cite{CSI_algo}:
\begin{equation} \label{cw}
	\tilde{x} = \arg \min_{\hat{x} \in D_{F_\theta}} \|\hat{x} - x\| - c \cdot L(F_{\theta}(\hat{x}), y),
\end{equation}
kde $c > 0$.

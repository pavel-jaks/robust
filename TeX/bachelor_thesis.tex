...
Další účelová funkce, která nachází uplatnění v klasifikačních problémech, je křížová entropie.
Lze ji vypočíst následovně:
\begin{equation}
...
a proto lze na výstup takovéto neuronové sítě a její značky (také pravděpodobnostní distribuce) aplikovat křížovou entropii.
...
Nejčastější metody učení neuronové sítě ve svém chodu pracují s gradientem účelové funkce.
K tomu je tedy nutné umět onen gradient spočíst.
A k tomu právě slouží \emph{algoritmus zpětného šíření chyby} (angl. \emph{backpropagation}).

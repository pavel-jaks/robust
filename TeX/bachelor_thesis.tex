...
\begin{equation}
	L(\theta) = - \frac{1}{N} \sum_{i=1}^N \sum_{j=1}^m y^{(i)}_j * \ln (F_\theta(x^{(i)})_j),
\end{equation}
...
Tento krok stranou ke stochastickému gradientnímu sestupu se ovšem dle \cite{Goodfellow}
...
že s postupem učení řád klesá.

% \subsection{Hybnost}

Další modifikací stochastického gradientního sestupu je zakomponování tzv. \emph{hybnosti}.
To uvádí na scénu novou proměnnou - \emph{rychlost} $v$ (z angl. \emph{velocity}),
...
Jinou modifikací stochastického gradientního sestupu, která je obdobou hybnosti, je \emph{Něstěrovova hybnost}.
Ta má následující předpis:
\begin{align}
...
desetkrát větší než u metod hybností. Byl-li by býval stejný, buď by se neuronové sítě pomocí metod hybností
vůbec nic nenaučili (pro řád učení $10^{-2}$), nebo by stochastický gradientní sestup nedosahoval takovývh výkonů
(pro řád učení $10^{-3}$).

Existují ale i další algoritmy, které stojí za zmínění se. Jedná se o algoritmy s přizpůsobivým řádem učení.

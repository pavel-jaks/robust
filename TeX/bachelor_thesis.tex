...
ve~smyslu $L^p$ normy a druhý člen je vážená účelová funkce, kde onu váhu reprezentuje hodnota $\lambda$,
kterou ladíme.
...
a za normu určující kouli o poloměru $\kappa$ okolo vzorků byla zvolena $L^{\infty}$ norma.
...
Žádný z útoků nebyl pro žádný vzorek úspěšný.
...
\begin{table}
	...
	\begin{tabular}{| c | c || c | c | c | c |}
		...
		$L^\infty$ & Jednou & 0 \% & 0 \% & 0 \% & 0 \% \\
		\hline
		$L^1$ & Jednou & 0 \% & 0 \% & 87 \% & 100 \% \\
		\hline
		$L^2$ & Jednou & 0 \% & 85 \% & 79 \% & 77 \% \\
		\hline
	\end{tabular}
...
Tedy, jak je vidět z tabulky \ref{cw_success_robust}, robustní učení neuchránilo síť od CW útoku provedeného v $L^1$ nebo $L^2$ normě.
Útok v $L^1$ normě při volbě hodnoty parametru $\lambda = 100$ je dokonce úspěšnější, je-li prováděn na robustně učené síti.
